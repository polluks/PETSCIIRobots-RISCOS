...
convertToPETSCII(CINEMA_MESSAGE);
    ...
    i = 0;
    for (; i < PLATFORM_MAP_WINDOW_TILES_HEIGHT; i++) {
...
int Y;
for (Y = 0; Y != 12; Y++) {
writeToScreenMemory((PLATFORM_SCREEN_HEIGHT - 32) / 2 / 8 * SCREEN_WIDTH_IN_CHARACTERS + (PLATFORM_SCREEN_WIDTH - 320) / 2 / 8 + Y, LOAD_MSG2[Y], 10, 0);
}
name = CALC_MAP_NAME();
for (Y = 0; Y != 16; Y++) {
writeToScreenMemory((PLATFORM_SCREEN_HEIGHT - 32) / 2 / 8 * SCREEN_WIDTH_IN_CHARACTERS + (PLATFORM_SCREEN_WIDTH - 320) / 2 / 8 + 12 + Y, name[Y], 10, 0);
}
}
...
if (BORDER != 0) {
BORDER--;
platformFadeScreen(15 - BORDER, true);
}
...
CYCLES++;
if (CYCLES != platformFramesPerSecond()) {
return;
}
CYCLES = 0;
SECONDS++;
...
{
bool done;
platformRenderFrame(false);
done = false;
while (!done && !platformQuit) {
uint8_t A;
uint16_t B;
#ifdef INACTIVITY_TIMEOUT_GAME
...
KEY_REPEAT(platformIsKeyOrJoystickPressed(CONTROL == 2 ? true : false));
A = platformReadKeyboard();
B = platformReadJoystick(CONTROL == 2 ? true : false);
if (A != 0xff) {
#ifdef INACTIVITY_TIMEOUT_GAME
...
KEYTIMER = 5;
if (A == KEY_CONFIG[KEY_CURSOR_RIGHT] || A == KEY_CONFIG[KEY_MOVE_RIGHT]) {
UNIT = 0;
MOVE_TYPE = 1;
REQUEST_WALK_RIGHT();
AFTER_MOVE();
} else if (A == KEY_CONFIG[KEY_CURSOR_LEFT] || A == KEY_CONFIG[KEY_MOVE_LEFT]) {
UNIT = 0;
MOVE_TYPE = 1;
REQUEST_WALK_LEFT();
AFTER_MOVE();
} else if (A == KEY_CONFIG[KEY_CURSOR_DOWN] || A == KEY_CONFIG[KEY_MOVE_DOWN]) {
UNIT = 0;
MOVE_TYPE = 1;
REQUEST_WALK_DOWN();
AFTER_MOVE();
} else if (A == KEY_CONFIG[KEY_CURSOR_UP] || A == KEY_CONFIG[KEY_MOVE_UP]) {
UNIT = 0;
MOVE_TYPE = 1;
REQUEST_WALK_UP();
AFTER_MOVE();
...
FIRE_RIGHT();
KEYTIMER = 20;
} else if (A == KEY_CONFIG[KEY_PAUSE]) {
done = PAUSE_GAME();
} else if (A == KEY_CONFIG[KEY_CHEAT]) {
CHEATER();
CLEAR_KEY_BUFFER();
} else if (A == KEY_CONFIG[KEY_MUSIC]) {
TOGGLE_MUSIC();
CLEAR_KEY_BUFFER();
}
#ifdef PLATFORM_LIVE_MAP_SUPPORT
...
if ((CONTROL == 2 && (B & JoystickPlay) == 0) ||
                (CONTROL != 2 && (B & JoystickBlue) == 0)) {
if (CONTROL != 2 && B & JoystickRed) {
if (B & JoystickLeft) {
FIRE_LEFT();
KEYTIMER = 20;
} else if (B & JoystickRight) {
FIRE_RIGHT();
KEYTIMER = 20;
} else if (B & JoystickUp) {
FIRE_UP();
KEYTIMER = 20;
} else if (B & JoystickDown) {
FIRE_DOWN();
KEYTIMER = 20;
}
} else {
...
if (CONTROL == 2) {
if (B & JoystickPlay) {
if (B & JoystickReverse) {
CYCLE_ITEM();
KEYTIMER = 15;
}
if (B & JoystickForward) {
CYCLE_WEAPON();
KEYTIMER = 15;
}
#ifdef PLATFORM_LIVE_MAP_SUPPORT
...
platformPlayModule(ModuleSoundFX);
#else
...
platformPlayModule(MUSIC_ON == 1 ? LEVEL_MUSIC[SELECTED_MAP] : ModuleSoundFX);
#else
...
while (!platformQuit) {
uint16_t B;
uint8_t A = platformReadKeyboard();
B = platformReadJoystick(CONTROL == 2 ? true : false);
...
PLAY_SOUND(15);
return false;
} else if (A == KEY_CONFIG[KEY_YES] || (B & JoystickRed)) {
UNIT_TYPE[0] = 0;
PLAY_SOUND(15);
GOM4();
return true;
...
platformSetCursorShape(ShapeUse);
#endif
...
MOVTEMP_UY = MAP_Y;
GET_TILE_FROM_MAP();
return (TILE_ATTRIB[TILE] & 0x01) == 0x01;
}
...
platformSetCursorShape(ShapeSearch);
#endif
...
writeToScreenMemory((SCREEN_HEIGHT_IN_CHARACTERS - 1) * SCREEN_WIDTH_IN_CHARACTERS + 9 + SEARCHBAR, 46, 10, 0);
}
#ifdef PLATFORM_CURSOR_SUPPORT
...
TEMP_A = UNIT_TYPE[UNIT_FIND];
TEMP_B = UNIT_A[UNIT_FIND];
...
platformShowCursor(CURSOR_X, CURSOR_Y);
#ifdef PLATFORM_LIVE_MAP_SUPPORT
...
A = platformReadKeyboard();
B = platformReadJoystick(CONTROL == 2 ? true : false);
if (A == KEY_CONFIG[KEY_CURSOR_RIGHT] || A == KEY_CONFIG[KEY_MOVE_RIGHT] || (B & JoystickRight)) {
UNIT_DIRECTION[0] = 3;
CURSOR_X++;
#ifdef PLATFORM_CURSOR_SUPPORT
...
return;
} else if (A == KEY_CONFIG[KEY_CURSOR_LEFT] || A == KEY_CONFIG[KEY_MOVE_LEFT] || (B & JoystickLeft)) {
UNIT_DIRECTION[0] = 2;
CURSOR_X--;
#ifdef PLATFORM_CURSOR_SUPPORT
...
return;
} else if (A == KEY_CONFIG[KEY_CURSOR_DOWN] || A == KEY_CONFIG[KEY_MOVE_DOWN] || (B & JoystickDown)) {
UNIT_DIRECTION[0] = 1;
CURSOR_Y++;
#ifdef PLATFORM_CURSOR_SUPPORT
...
return;
} else if (A == KEY_CONFIG[KEY_CURSOR_UP] || A == KEY_CONFIG[KEY_MOVE_UP] || (B & JoystickUp)) {
UNIT_DIRECTION[0] = 0;
CURSOR_Y--;
#ifdef PLATFORM_CURSOR_SUPPORT
...
platformSetCursorShape(ShapeMove);
#endif
...
MOVTEMP_UY = MAP_Y;
GET_TILE_FROM_MAP();
if (TILE_ATTRIB[TILE] & 0x20) {
CHECK_FOR_UNIT();
if (UNIT_FIND == 255) {
int A;
PLAY_SOUND(6);
MOVTEMP_D = MAP_SOURCE[0];
MAP_SOURCE[0] = MOVTEMP_O;
MAP_X = MOVTEMP_X;
MAP_Y = MOVTEMP_Y;
GET_TILE_FROM_MAP();
A = MOVTEMP_D;
...
if (LIVE_MAP_ON == 1) {
platformRenderLiveMapTile(MAP, MOVTEMP_X, MOVTEMP_Y);
platformRenderLiveMapTile(MAP, MOVTEMP_UX, MOVTEMP_UY);
}
#endif
...
{
MAP_WINDOW_X = MIN(MAX(UNIT_LOC_X[0] - PLATFORM_MAP_WINDOW_TILES_WIDTH / 2, 0), 128 - PLATFORM_MAP_WINDOW_TILES_WIDTH);
MAP_WINDOW_Y = MIN(MAX(UNIT_LOC_Y[0] - PLATFORM_MAP_WINDOW_TILES_HEIGHT / 2, 0), 64 - PLATFORM_MAP_WINDOW_TILES_HEIGHT);
REDRAW_WINDOW = 1;
}
...
if (UNIT_TILE[X] == 130 || UNIT_TILE[X] == 134) {
if (MAP_PRECALC[Y] != 0) {
continue;
}
}
...
MAP_SOURCE = MAP + ((MAP_WINDOW_Y << 7) + MAP_WINDOW_X);
    ...
    TEMP_Y = 0;
    PRECALC_COUNT = 0;
    for (; TEMP_Y != PLATFORM_MAP_WINDOW_TILES_HEIGHT; TEMP_Y++, MAP_SOURCE += 128 - PLATFORM_MAP_WINDOW_TILES_WIDTH, PRECALC_COUNT += PLATFORM_MAP_WINDOW_TILES_WIDTH - PLATFORM_MAP_WINDOW_TILES_WIDTH) {
    uint16_t TEMP_X;

    TEMP_X = 0;
    for (; TEMP_X != PLATFORM_MAP_WINDOW_TILES_WIDTH; TEMP_X++, MAP_SOURCE++, PRECALC_COUNT++) {
uint8_t VARIANT;
uint8_t FG_TILE;
uint8_t FG_VARIANT;
TILE = MAP_SOURCE[0];
...
FG_TILE = MAP_PRECALC[PRECALC_COUNT];
FG_VARIANT = 0;
...
if (FG_TILE != 0) {
platformRenderTiles(TILE, FG_TILE, TEMP_X * 24, TEMP_Y * 24, VARIANT, FG_VARIANT);
...
platformRenderTile(TILE, TEMP_X * 24, TEMP_Y * 24, VARIANT, false);
PREVIOUS_MAP_BACKGROUND[PRECALC_COUNT] = TILE;
PREVIOUS_MAP_BACKGROUND_VARIANT[PRECALC_COUNT] = VARIANT;
PREVIOUS_MAP_FOREGROUND[PRECALC_COUNT] = FG_TILE;
PREVIOUS_MAP_FOREGROUND_VARIANT[PRECALC_COUNT] = FG_VARIANT;
switch (TILE) {
case 20: {
                        platformWaitForScreenMemoryAccess();
                        platformWriteToScreenMemory(MAP_CHART[TEMP_Y] + TEMP_X + TEMP_X + TEMP_X + SCREEN_WIDTH_IN_CHARACTERS + 1, CINEMA_MESSAGE[CINEMA_STATE], 1, 0);
                        platformWriteToScreenMemory(MAP_CHART[TEMP_Y] + TEMP_X + TEMP_X + TEMP_X + SCREEN_WIDTH_IN_CHARACTERS + 2, CINEMA_MESSAGE[CINEMA_STATE + 1], 1, 0);
                        break;
                    }
                    case 21: {
                        platformWaitForScreenMemoryAccess();
                        platformWriteToScreenMemory(MAP_CHART[TEMP_Y] + TEMP_X + TEMP_X + TEMP_X + SCREEN_WIDTH_IN_CHARACTERS + 0, CINEMA_MESSAGE[CINEMA_STATE + 2], 1, 0);
                        platformWriteToScreenMemory(MAP_CHART[TEMP_Y] + TEMP_X + TEMP_X + TEMP_X + SCREEN_WIDTH_IN_CHARACTERS + 1, CINEMA_MESSAGE[CINEMA_STATE + 3], 1, 0);
                        platformWriteToScreenMemory(MAP_CHART[TEMP_Y] + TEMP_X + TEMP_X + TEMP_X + SCREEN_WIDTH_IN_CHARACTERS + 2, CINEMA_MESSAGE[CINEMA_STATE + 4], 1, 0);
                        break;
                    }
                    case 22: {
                        platformWaitForScreenMemoryAccess();
                        platformWriteToScreenMemory(MAP_CHART[TEMP_Y] + TEMP_X + TEMP_X + TEMP_X + SCREEN_WIDTH_IN_CHARACTERS + 0, CINEMA_MESSAGE[CINEMA_STATE + 5], 1, 0);
                        break;
                    }
                    default:
                        break;
...
{
    uint8_t TEMP_Y;
    uint8_t PRECALC_COUNT;
MAP_PRE_CALCULATE();
REDRAW_WINDOW = 0;

    TEMP_Y = 0;
    PRECALC_COUNT = 0;
    for (; TEMP_Y != PLATFORM_MAP_WINDOW_TILES_HEIGHT; TEMP_Y++) {
    uint8_t TEMP_X;

    TEMP_X = 0;
    for (; TEMP_X != PLATFORM_MAP_WINDOW_TILES_WIDTH; TEMP_X++) {
...
platformRenderTiles(TILE, FG_TILE, TEMP_X * 24, TEMP_Y * 24, 0, variant);
} else {
platformRenderTile(TILE, TEMP_X * 24, TEMP_Y * 24, 0, false);
}
#else
...
void DRAW_LIVE_MAP()
{
platformRenderLiveMapUnits(MAP, UNIT_TYPE, UNIT_LOC_X, UNIT_LOC_Y, LIVE_MAP_PLAYER_BLINK < 128 ? 1 : 0, LIVE_MAP_ROBOTS_ON == 1 ? true : false);
...
platformRenderTile(TILE, x * 24, y * 24, 0, false);
}
...
platformRenderTile(TILE, x * 24, y * 24, variant, true);
}
...
writeToScreenMemory(destination + 0, SCREEN_MEMORY[destination + 0] ^= 0x80, 10, 0);
writeToScreenMemory(destination + 1, SCREEN_MEMORY[destination + 1] ^= 0x80, 10, 0);
writeToScreenMemory(destination + 2, SCREEN_MEMORY[destination + 2] ^= 0x80, 10, 0);
writeToScreenMemory(destination + 40, SCREEN_MEMORY[destination + 40] ^= 0x80, 10, 0);
writeToScreenMemory(destination + 41, SCREEN_MEMORY[destination + 41] ^= 0x80, 10, 0);
writeToScreenMemory(destination + 42, SCREEN_MEMORY[destination + 42] ^= 0x80, 10, 0);
writeToScreenMemory(destination + 80, SCREEN_MEMORY[destination + 80] ^= 0x80, 10, 0);
writeToScreenMemory(destination + 81, SCREEN_MEMORY[destination + 81] ^= 0x80, 10, 0);
writeToScreenMemory(destination + 82, SCREEN_MEMORY[destination + 82] ^= 0x80, 10, 0);
}
#endif
...
void DECWRITE(uint16_t destination, uint8_t color)
{
    ...
    X = 2;
    for (; X >= 0; X--) {
writeToScreenMemory(destination + X, 0x30 + (DECNUM % 10), color, 0);
DECNUM /= 10;
}
}
...
{
uint8_t * tileset;
tileset = platformLoadTileset();
DESTRUCT_PATH = tileset + 2 + 0 * 256;
TILE_ATTRIB = tileset + 2 + 1 * 256;
#ifdef PLATFORM_SPRITE_SUPPORT

platformGenerateTiles(0, TILE_ATTRIB);
#else
...
platformGenerateTiles(TILE_DATA_TL, TILE_ATTRIB);
#endif
...
{
platformLoadMap((Map)SELECTED_MAP, UNIT_TYPE);
}
...
platformDisplayImage(ImageGame);
writeToScreenMemory(25 * SCREEN_WIDTH_IN_CHARACTERS - 6, 0x71, 15, 0);
writeToScreenMemory(25 * SCREEN_WIDTH_IN_CHARACTERS - 5, 0x71, 15, 0);
writeToScreenMemory(25 * SCREEN_WIDTH_IN_CHARACTERS - 4, 0x71, 12, 0);
writeToScreenMemory(25 * SCREEN_WIDTH_IN_CHARACTERS - 3, 0x71, 12, 0);
writeToScreenMemory(25 * SCREEN_WIDTH_IN_CHARACTERS - 2, 0x71, 9, 0);
writeToScreenMemory(25 * SCREEN_WIDTH_IN_CHARACTERS - 1, 0x71, 9, 0);
#else

DECOMPRESS_SCREEN(SCR_TEXT, 10);
#endif
...
row = SCREEN_MEMORY + MENU_CHART[0];

    Y = 0;
    i = 0;
    for (; Y < PLATFORM_INTRO_OPTIONS; Y++, row += SCREEN_WIDTH_IN_CHARACTERS) {
    int X;
...
platformDisplayImage(ImageIntro);
#else

DECOMPRESS_SCREEN(INTRO_TEXT, 10);
#endif
...
platformDisplayImage(ImageGameOver);
#else

DECOMPRESS_SCREEN(SCR_ENDGAME, 10);
#endif
...
name = CALC_MAP_NAME();
    ...
    Y = 0;
    for (; Y != 16; Y++) {
writeToScreenMemory(7 * SCREEN_WIDTH_IN_CHARACTERS + 22 + Y, name[Y], 4, 0);
}
DECNUM = HOURS;
...
writeToScreenMemory(9 * SCREEN_WIDTH_IN_CHARACTERS + 21, 32, 4, 0);
writeToScreenMemory(9 * SCREEN_WIDTH_IN_CHARACTERS + 24, 58, 4, 0);
writeToScreenMemory(9 * SCREEN_WIDTH_IN_CHARACTERS + 27, 58, 4, 0);
DECNUM = 0;
for (X = 1; X != 28; X++) {
if (UNIT_TYPE[X] != 0) {
DECNUM++;
...
WORD = DIFF_LEVEL_WORDS + (DIFF_LEVEL * 6);
for (X = 0; X < 6; X++) {
writeToScreenMemory(15 * SCREEN_WIDTH_IN_CHARACTERS + 22 + X, WORD[X], 4, 0);
}
}
...
while (!platformQuit) {
if (*source != 96) {
writeToScreenMemory(destination, *source, color, 0);
} else {
    int X;
source++;
RPT = *source;
source++;
    ...
    X = *source;
    for (; X >= 0; X--) {
writeToScreenMemory(destination++, RPT, color, 0);
}
destination--;
}
...
int Y;
int health;
TEMP_A = UNIT_HEALTH[0] >> 1;
Y = 0;
while (Y != TEMP_A) {
writeToScreenMemory(24 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y++, 0x66, 10, 0);
}
if (UNIT_HEALTH[0] & 0x01) {
writeToScreenMemory(24 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y++, 0x5C, 10, 0);
}
while (Y != 6) {
writeToScreenMemory(24 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y++, 0x20, 10, 0);
}
#ifdef PLATFORM_IMAGE_SUPPORT

health = 5 - MIN(TEMP_A, 5);
platformRenderHealth(health, PLATFORM_SCREEN_WIDTH - 48, 131 + (health >> 1));
#endif
...
platformRenderItem(5, PLATFORM_SCREEN_WIDTH - 48, 54);
DECNUM = INV_BOMBS;
DECWRITE(11 * SCREEN_WIDTH_IN_CHARACTERS - 3, 1);
    ...
    Y = 0;
    for (; Y != 6; Y++) {
writeToScreenMemory(9 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, TBOMB1A[Y], 10, 0);
writeToScreenMemory(10 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, TBOMB1B[Y], 10, 0);
writeToScreenMemory(11 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, TBOMB1C[Y], 10, 0);
writeToScreenMemory(12 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, TBOMB1D[Y], 10, 0);
}
DECNUM = INV_BOMBS;
DECWRITE(13 * SCREEN_WIDTH_IN_CHARACTERS - 3, 10);
#endif
...
platformRenderItem(3, PLATFORM_SCREEN_WIDTH - 48, 54);
DECNUM = INV_EMP;
DECWRITE(11 * SCREEN_WIDTH_IN_CHARACTERS - 3, 1);
#else
    ...
    Y = 0;
    for (; Y != 6; Y++) {
writeToScreenMemory(9 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, EMP1A[Y], 10, 0);
writeToScreenMemory(10 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, EMP1B[Y], 10, 0);
writeToScreenMemory(11 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, EMP1C[Y], 10, 0);
writeToScreenMemory(12 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, EMP1D[Y], 10, 0);
}
DECNUM = INV_EMP;
DECWRITE(13 * SCREEN_WIDTH_IN_CHARACTERS - 3, 10);
#endif
...
platformRenderItem(2, PLATFORM_SCREEN_WIDTH - 48, 54);
DECNUM = INV_MEDKIT;
DECWRITE(11 * SCREEN_WIDTH_IN_CHARACTERS - 3, 1);
#else
    ...
    Y = 0;
    for (; Y != 6; Y++) {
writeToScreenMemory(9 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, MED1A[Y], 10, 0);
writeToScreenMemory(10 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, MED1B[Y], 10, 0);
writeToScreenMemory(11 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, MED1C[Y], 10, 0);
writeToScreenMemory(12 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, MED1D[Y], 10, 0);
}
DECNUM = INV_MEDKIT;
DECWRITE(13 * SCREEN_WIDTH_IN_CHARACTERS - 3, 10);
#endif
...
platformRenderItem(4, PLATFORM_SCREEN_WIDTH - 48, 54);
DECNUM = INV_MAGNET;
DECWRITE(11 * SCREEN_WIDTH_IN_CHARACTERS - 3, 1);
#else
    ...
    Y = 0;
    for (; Y != 6; Y++) {
writeToScreenMemory(9 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, MAG1A[Y], 10, 0);
writeToScreenMemory(10 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, MAG1B[Y], 10, 0);
writeToScreenMemory(11 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, MAG1C[Y], 10, 0);
writeToScreenMemory(12 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, MAG1D[Y], 10, 0);
}
DECNUM = INV_MAGNET;
DECWRITE(13 * SCREEN_WIDTH_IN_CHARACTERS - 3, 10);
#endif
...
platformClearRect(PLATFORM_SCREEN_WIDTH - 48, 48, 48, 40);
#else

platformClearRect(PLATFORM_SCREEN_WIDTH - 48, 64, 48, 40);
#endif
...
platformRenderItem(1, PLATFORM_SCREEN_WIDTH - 48, 13);
DECNUM = AMMO_PLASMA;
DECWRITE(5 * SCREEN_WIDTH_IN_CHARACTERS - 3, 1);
    ...
    Y = 0;
    for (; Y != 6; Y++) {
writeToScreenMemory(2 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, WEAPON1A[Y], 10, 0);
writeToScreenMemory(3 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, WEAPON1B[Y], 10, 0);
writeToScreenMemory(4 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, WEAPON1C[Y], 10, 0);
writeToScreenMemory(5 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, WEAPON1D[Y], 10, 0);
}
DECNUM = AMMO_PLASMA;
DECWRITE(6 * SCREEN_WIDTH_IN_CHARACTERS - 3, 10);
#endif
...
platformRenderItem(0, PLATFORM_SCREEN_WIDTH - 48, 13);
DECNUM = AMMO_PISTOL;
DECWRITE(5 * SCREEN_WIDTH_IN_CHARACTERS - 3, 1);
#else
    ...
    Y = 0;
    for (; Y != 6; Y++) {
writeToScreenMemory(2 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, PISTOL1A[Y], 10, 0);
writeToScreenMemory(3 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, PISTOL1B[Y], 10, 0);
writeToScreenMemory(4 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, PISTOL1C[Y], 10, 0);
writeToScreenMemory(5 * SCREEN_WIDTH_IN_CHARACTERS - 6 + Y, PISTOL1D[Y], 10, 0);
}
DECNUM = AMMO_PISTOL;
DECWRITE(6 * SCREEN_WIDTH_IN_CHARACTERS - 3, 10);
#endif
...
platformClearRect(PLATFORM_SCREEN_WIDTH - 48, 8, 48, 32);
#else

platformClearRect(PLATFORM_SCREEN_WIDTH - 48, 16, 48, 32);
#endif
...
if (KEYS & 0x01) {
platformRenderKey(0, PLATFORM_SCREEN_WIDTH - 48, 106);
}
if (KEYS & 0x02) {
platformRenderKey(1, PLATFORM_SCREEN_WIDTH - 32, 106);
}
if (KEYS & 0x04) {
platformRenderKey(2, PLATFORM_SCREEN_WIDTH - 16, 106);
}
#else

if (KEYS & 0x01) {
writeToScreenMemory(16 * SCREEN_WIDTH_IN_CHARACTERS - 6, 0x63, 10, 0);
writeToScreenMemory(16 * SCREEN_WIDTH_IN_CHARACTERS - 5, 0x4D, 10, 0);
writeToScreenMemory(17 * SCREEN_WIDTH_IN_CHARACTERS - 6, 0x41, 10, 0);
writeToScreenMemory(17 * SCREEN_WIDTH_IN_CHARACTERS - 5, 0x67, 10, 0);
}
if (KEYS & 0x02) {
writeToScreenMemory(16 * SCREEN_WIDTH_IN_CHARACTERS - 4, 0x63, 10, 0);
writeToScreenMemory(16 * SCREEN_WIDTH_IN_CHARACTERS - 3, 0x4D, 10, 0);
writeToScreenMemory(17 * SCREEN_WIDTH_IN_CHARACTERS - 4, 0x53, 10, 0);
writeToScreenMemory(17 * SCREEN_WIDTH_IN_CHARACTERS - 3, 0x67, 10, 0);
}
if (KEYS & 0x04) {
writeToScreenMemory(16 * SCREEN_WIDTH_IN_CHARACTERS - 2, 0x63, 10, 0);
writeToScreenMemory(16 * SCREEN_WIDTH_IN_CHARACTERS - 1, 0x4D, 10, 0);
writeToScreenMemory(17 * SCREEN_WIDTH_IN_CHARACTERS - 2, 0x2A, 10, 0);
writeToScreenMemory(17 * SCREEN_WIDTH_IN_CHARACTERS - 1, 0x67, 10, 0);
}
#endif
...
BACKGROUND_TASKS();
    ...
    X = 0;
    for (; X != 11; X++) {
writeToScreenMemory((SCREEN_HEIGHT_IN_CHARACTERS - 3 - 3) / 2 * SCREEN_WIDTH_IN_CHARACTERS + (SCREEN_WIDTH_IN_CHARACTERS - 7 - 11) / 2 + X, GAMEOVER1[X], 10, 0);
writeToScreenMemory(((SCREEN_HEIGHT_IN_CHARACTERS - 3 - 3) / 2 + 1) * SCREEN_WIDTH_IN_CHARACTERS + (SCREEN_WIDTH_IN_CHARACTERS - 7 - 11) / 2 + X, GAMEOVER2[X], 10, 0);
writeToScreenMemory(((SCREEN_HEIGHT_IN_CHARACTERS - 3 - 3) / 2 + 2) * SCREEN_WIDTH_IN_CHARACTERS + (SCREEN_WIDTH_IN_CHARACTERS - 7 - 11) / 2 + X, GAMEOVER3[X], 10, 0);
}
platformRenderFrame(false);
#ifdef PLATFORM_MODULE_BASED_AUDIO
...
platformStopModule();
if (MUSIC_ON == 1) {
platformLoadModule(UNIT_TYPE[0] != 0 ? ModuleWin : ModuleLose);
}
#else
...
platformClearKeyBuffer();
    ...
    frames = 0;
    for (; platformReadKeyboard() == 0xff && platformReadJoystick(CONTROL == 2 ? true : false) == 0 && !platformQuit && frames < 5 * platformFramesPerSecond(); frames++) {
#else
    while (platformReadKeyboard() == 0xff && platformReadJoystick(CONTROL == 2 ? true : false) == 0 && !platformQuit) {
#endif
...
platformStartFadeScreen(0x000, 15);
platformFadeScreen(0, false);
...
platformFadeScreen(15, false);
    ...
    frames = 0;
    for (; platformReadKeyboard() == 0xff && platformReadJoystick(CONTROL == 2 ? true : false) == 0 && !platformQuit && frames < 15 * platformFramesPerSecond(); frames++) {
#else
    while (platformReadKeyboard() == 0xff && platformReadJoystick(CONTROL == 2 ? true : false) == 0 && !platformQuit) {
#endif
...
writeToScreenMemory((SCREEN_HEIGHT_IN_CHARACTERS - 1) * SCREEN_WIDTH_IN_CHARACTERS + PRINTX, text[Y], 10, 0);
PRINTX++;
}
}
}
...
void SCROLL_INFO()
{
platformCopyRect(0, PLATFORM_SCREEN_HEIGHT - 16, 0, PLATFORM_SCREEN_HEIGHT - 24, PLATFORM_SCREEN_WIDTH - 56, 16);
platformClearRect(0, PLATFORM_SCREEN_HEIGHT - 8, PLATFORM_SCREEN_WIDTH - 56, 8);
platformRenderFrame(true);
}
...
uint16_t B;
uint8_t A = platformReadKeyboard();
B = platformReadJoystick(false);
if (A != 0xff || B != 0) {
if (A == KEY_CONFIG[KEY_CURSOR_DOWN] || A == KEY_CONFIG[KEY_MOVE_DOWN] || (B & JoystickDown)) {
...
PLAY_SOUND(15);
                       ...
                       A == KEY_CONFIG[KEY_RETURN] || (B & JoystickRed)) {
done = EXEC_COMMAND();
}
#ifdef PLATFORM_MODULE_BASED_AUDIO
...
platformPlayModule(MUSIC_ON == 1 ? ModuleIntro : ModuleSoundFX);
#else
...
writeToScreenMemory(5 * SCREEN_WIDTH_IN_CHARACTERS + 4 + X, CONTROLTEXT[Y] | 0x80, 10, 0);
#endif
...
if (SELECTED_MAP == PLATFORM_MAP_COUNT) {
SELECTED_MAP = 0;
}
DISPLAY_MAP_NAME();
}
...
writeToScreenMemory(9 * SCREEN_WIDTH_IN_CHARACTERS + 2 + Y, name[Y], 10, 0);
#endif
...
char* CALC_MAP_NAME()
{
return MAP_NAMES + (SELECTED_MAP << 4);
}
...
if (reverse) {
platformSetHighlightedMenuRow(MENUY);
}
#else
...
writeToScreenMemory(MENU_CHART[MENUY] + Y, SCREEN_MEMORY[MENU_CHART[MENUY] + Y], reverse ? 14 : 15, 5);
    ...
    Y = 0;
    for (; Y != 10; Y++) {
writeToScreenMemory(MENU_CHART[MENUY] + Y, SCREEN_MEMORY[MENU_CHART[MENUY] + Y] ^ 0x80, 10, 0);
}
#endif
...
platformRenderFace(DIFF_LEVEL, 234, 75);
#else

Y = FACE_LEVEL[DIFF_LEVEL];
writeToScreenMemory(5 * SCREEN_WIDTH_IN_CHARACTERS + 21, ROBOT_FACE[Y++], 10, 0);
writeToScreenMemory(5 * SCREEN_WIDTH_IN_CHARACTERS + 22, ROBOT_FACE[Y++], 10, 0);
writeToScreenMemory(5 * SCREEN_WIDTH_IN_CHARACTERS + 23, ROBOT_FACE[Y++], 10, 0);
writeToScreenMemory(5 * SCREEN_WIDTH_IN_CHARACTERS + 25, ROBOT_FACE[Y++], 10, 0);
writeToScreenMemory(5 * SCREEN_WIDTH_IN_CHARACTERS + 26, ROBOT_FACE[Y++], 10, 0);
writeToScreenMemory(5 * SCREEN_WIDTH_IN_CHARACTERS + 27, ROBOT_FACE[Y++], 10, 0);
writeToScreenMemory(6 * SCREEN_WIDTH_IN_CHARACTERS + 23, ROBOT_FACE[Y++], 10, 0);
writeToScreenMemory(6 * SCREEN_WIDTH_IN_CHARACTERS + 25, ROBOT_FACE[Y], 10, 0);
#endif
...
platformFillRect(0, 0, MAP_WINDOW_WIDTH, MAP_WINDOW_HEIGHT, 15);
INVALIDATE_PREVIOUS_MAP();
#else
    ...
    Y = 0;
    for (; Y != 33; Y++) {
writeToScreenMemory(0 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[0 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(1 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[1 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(2 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[2 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(3 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[3 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(4 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[4 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(5 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[5 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(6 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[6 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(7 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[7 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(8 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[8 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(9 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[9 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(10 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[10 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(11 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[11 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(12 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[12 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(13 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[13 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(14 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[14 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(15 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[15 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(16 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[16 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(17 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[17 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(18 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[18 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(19 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[19 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
writeToScreenMemory(20 * SCREEN_WIDTH_IN_CHARACTERS + Y, SCREEN_MEMORY[20 * SCREEN_WIDTH_IN_CHARACTERS + Y] ^ 0x80, 10, 0);
}
#endif
...
TILE_DATA_MM[20] = TILE_DATA_MR[20];
TILE_DATA_MR[20] = TILE_DATA_ML[21];
TILE_DATA_ML[21] = TILE_DATA_MM[21];
TILE_DATA_MM[21] = TILE_DATA_MR[21];
TILE_DATA_MR[21] = TILE_DATA_ML[22];
TILE_DATA_ML[22] = CINEMA_MESSAGE[CINEMA_STATE];
CINEMA_STATE++;
if (CINEMA_STATE == 197) {
CINEMA_STATE = 0;
}
...
platformUpdateTiles(TILE_DATA_TL, tiles, 11);
#ifdef OPTIMIZED_MAP_RENDERING
...
ELEVATOR_MAX_FLOOR = UNIT_D[UNIT];

    Y = 0;
    A = 0x31;
    for (; Y != ELEVATOR_MAX_FLOOR; A++, Y++) {
writeToScreenMemory((SCREEN_HEIGHT_IN_CHARACTERS - 1) * SCREEN_WIDTH_IN_CHARACTERS + 6 + Y, A, 10, 0);
}
ELEVATOR_CURRENT_FLOOR = UNIT_C[UNIT];
ELEVATOR_INVERT();
platformRenderFrame(false);
while (!platformQuit) {
uint8_t A;
...
if (A == KEY_CONFIG[KEY_LIVE_MAP] || (CONTROL == 2 && (B & JoystickPlay) && (B & JoystickLeft))) {
TOGGLE_LIVE_MAP();
if (LIVE_MAP_ON == 0) {
DRAW_MAP_WINDOW();
}
CLEAR_KEY_BUFFER();
} else if (A == KEY_CONFIG[KEY_LIVE_MAP_ROBOTS] || (CONTROL == 2 && (B & JoystickPlay) && (B & JoystickDown))) {
TOGGLE_LIVE_MAP_ROBOTS();
CLEAR_KEY_BUFFER();
} else
#endif
            if (A == KEY_CONFIG[KEY_CURSOR_LEFT] || A == KEY_CONFIG[KEY_MOVE_LEFT] || (B & JoystickLeft)) {
ELEVATOR_DEC();
} else if (A == KEY_CONFIG[KEY_CURSOR_RIGHT] || A == KEY_CONFIG[KEY_MOVE_RIGHT] || (B & JoystickRight)) {
ELEVATOR_INC();
} else if (A == KEY_CONFIG[KEY_CURSOR_DOWN] || A == KEY_CONFIG[KEY_MOVE_DOWN] || (B & JoystickDown)) {
SCROLL_INFO();
SCROLL_INFO();
...
{
writeToScreenMemory((SCREEN_HEIGHT_IN_CHARACTERS - 1) * SCREEN_WIDTH_IN_CHARACTERS + 5 + ELEVATOR_CURRENT_FLOOR, SCREEN_MEMORY[(SCREEN_HEIGHT_IN_CHARACTERS - 1) * SCREEN_WIDTH_IN_CHARACTERS + 5 + ELEVATOR_CURRENT_FLOOR] ^ 0x80, 10, 0);
}
...
SET_CUSTOM_KEYS();
    ...
    int Y;
uint8_t* STANDARD_CONTROLS = platformStandardControls();

    Y = 0;
    for (; Y != sizeof(KEY_CONFIG); Y++) {
...
DECOMPRESS_SCREEN(SCR_CUSTOM_KEYS, 10);
#endif

platformRenderFrame(false);
platformFadeScreen(15, false);
destination = 8 * SCREEN_WIDTH_IN_CHARACTERS + 17;
...
if (SCREEN_SHAKE == 1) {
platformCopyRect(8, 0, 0, 0, PLATFORM_SCREEN_WIDTH - 56, PLATFORM_SCREEN_HEIGHT - 32);
platformRenderFrame(true);
#ifdef OPTIMIZED_MAP_RENDERING
...
if (FLASH_STATE != 1) {
    ...
    X = 0;
    for (; X != 6; X++) {
writeToScreenMemory(19 * SCREEN_WIDTH_IN_CHARACTERS - 6 + X, OUCH1[X], 10, 0);
writeToScreenMemory(20 * SCREEN_WIDTH_IN_CHARACTERS - 6 + X, OUCH2[X], 10, 0);
writeToScreenMemory(21 * SCREEN_WIDTH_IN_CHARACTERS - 6 + X, OUCH3[X], 10, 0);
}
#endif

platformStartFadeScreen(BORDER_COLOR, 15 - BORDER);
FLASH_STATE = 1;
}
} else {
if (FLASH_STATE != 0) {
    ...
    X = 0;
    for (; X != 6; X++) {
writeToScreenMemory(19 * SCREEN_WIDTH_IN_CHARACTERS - 6 + X, 32, 10, 0);
writeToScreenMemory(20 * SCREEN_WIDTH_IN_CHARACTERS - 6 + X, 32, 10, 0);
writeToScreenMemory(21 * SCREEN_WIDTH_IN_CHARACTERS - 6 + X, 32, 10, 0);
}
#endif

FLASH_STATE = 0;
platformStopFadeScreen();
}
}
}
...
UNIT_TILE[0] = (UNIT_TIMER_B[UNIT] & 0x01) + 252;
...
UNIT_LOC_X[0] = UNIT_C[UNIT];
UNIT_LOC_Y[0] = UNIT_D[UNIT];
UNIT_TYPE[UNIT] = 7;
CACULATE_AND_REDRAW();
}
}
...
platformPlaySample(sound);
#else

if (MUSIC_ON != 0 && SOUND_EFFECT == 0xFF) {
PATTERN_TEMP = CUR_PATTERN;
DATA_LINE_TEMP = DATA_LINE;
TEMPO_TEMP = TEMPO;
}
if (SOUND_EFFECT != 0xFF) {
if (sound > SOUND_EFFECT) {
return;
}
}
CUR_PATTERN = SOUND_LIBRARY[sound];
SOUND_EFFECT = sound;
DATA_LINE = 0;
#endif
...
PLAY_SOUND(4);
UNIT_D[UNIT_FIND] = UNIT_TYPE[UNIT_FIND];
UNIT_TYPE[UNIT_FIND] = 21;
...
if (UNIT_LOC_X[UNIT] > UNIT_LOC_X[0]) {
if (UNIT_LOC_X[UNIT] - UNIT_LOC_X[0] >= 6) {
return;
}
for (X = 28; X != 32; X++) {
if (UNIT_TYPE[X] == 0) {
UNIT_TYPE[X] = 14;
ROLLERBOT_AFTER_FIRE(X, 245);
return;
}
}
} else {
if (UNIT_LOC_X[0] - UNIT_LOC_X[UNIT] >= 6) {
return;
}
for (X = 28; X != 32; X++) {
if (UNIT_TYPE[X] == 0) {
...
if (UNIT_LOC_Y[UNIT] > UNIT_LOC_Y[0]) {
if (UNIT_LOC_Y[UNIT] - UNIT_LOC_Y[0] >= 4) {
return;
}
for (X = 28; X != 32; X++) {
if (UNIT_TYPE[X] == 0) {
UNIT_TYPE[X] = 12;
ROLLERBOT_AFTER_FIRE(X, 244);
return;
}
}
} else {
if (UNIT_LOC_Y[0] - UNIT_LOC_Y[UNIT] >= 4) {
return;
}
for (X = 28; X != 32; X++) {
if (UNIT_TYPE[X] == 0) {
...
GET_TILE_FROM_MAP();
MAP_SOURCE[0] = 246;
platformRenderLiveMapTile(MAP, MAP_X, MAP_Y);
MAP_SOURCE[0] = TILE;
}
#endif
...
{
GET_TILE_FROM_MAP();
return (TILE_ATTRIB[TILE] & 0x10) == 0x10;
}
...
if (TEMP_A != 131) {
if ((TILE_ATTRIB[TEMP_A] & 0x08) == 0x08) {
...
UNIT_TIMER_A[UNIT] = 5;
UNIT_A[UNIT] = 0;
} else if ((TILE_ATTRIB[TILE] & 0x10) != 0x10) {
UNIT_TYPE[UNIT] = 11;
UNIT_TILE[UNIT] = 248;
CHECK_FOR_WINDOW_REDRAW();
} else {
...
void ALTER_AI()
{
if (UNIT_TYPE[UNIT_FIND] == 2 || UNIT_TYPE[UNIT_FIND] == 3) {
UNIT_TYPE[UNIT_FIND] = 4;
}
}
...
if (LIVE_MAP_ON == 1) {
platformRenderLiveMapTile(MAP, UNIT_LOC_X[UNIT], UNIT_LOC_Y[UNIT]);
}
#endif
...
Y = ABS(UNIT_LOC_Y[UNIT] - UNIT_LOC_Y[0]);
PROX_DETECT = (X == 1 && Y == 0) || (X == 0 && Y == 1) ? 1 : 0;
}

void DOOR_CHECK_PROXIMITY()
{
int A = ABS(UNIT_LOC_X[UNIT] - UNIT_LOC_X[0]);
if (A >= 2) {
PROX_DETECT = 0;
return;
}
A = ABS(UNIT_LOC_Y[UNIT] - UNIT_LOC_Y[0]);
if (A >= 2) {
PROX_DETECT = 0;
return;
}
...
if ((UNIT_LOC_X[UNIT] != UNIT_LOC_X[0]) || // elevator X location, player X location
        ((UNIT_LOC_Y[UNIT] - 1) != UNIT_LOC_Y[0])) {
return;
}
...
MAP_Y = UNIT_LOC_Y[UNIT];
GET_TILE_FROM_MAP();
if ((TILE_ATTRIB[TILE] & MOVE_TYPE) == MOVE_TYPE) {
CHECK_FOR_UNIT();
if (UNIT_FIND == 255) {
UNIT_LOC_X[UNIT]++;
MOVE_RESULT = 1;
...
if (UNIT_TYPE[X] != 0 &&
            ...
            (UNIT_LOC_Y[X] == MAP_Y || // now compare vertical position
             (UNIT_LOC_Y[X] <= MAP_Y && (UNIT_LOC_Y[X] + UNIT_D[X]) >= MAP_Y))) {
UNIT_FIND = X;
return;
}
}
...
platformWriteToScreenMemory(address, value, color, yOffset);
#else

platformWriteToScreenMemory(address, value);
#endif
