...
#else
int assert(int cond, const char *fmt, ...) {
	(void)cond; (void)fmt;
	return 0;
}
#endif
...
ModPlayerStatus_t *ProcessMOD() { int i, s, ch;
	if(mp.tick == 0) {
		mp.skiporderrequest = -1;

		for(i = 0; i < mp.channels; i++) {
			const uint8_t *cell;
			int note_tmp, sample_tmp, eff_tmp, effval_tmp;

			mp.ch[i].vibrato.val = mp.ch[i].tremolo.val = 0;

			cell = mp.patterndata + 4 * (i + mp.channels * (mp.row + 64 * mp.ordertable[mp.order]));

			note_tmp = ((cell[0] << 8) | cell[1]) & 0xFFF;
			sample_tmp = (cell[0] & 0xF0) | (cell[2] >> 4);
			eff_tmp = cell[2] & 0x0F;
			effval_tmp = cell[3];
	...
	for(i = 0; i < mp.channels; i++) {
		...
		vol = mp.ch[i].volume + (mp.ch[i].tremolo.val >> 6);
...
ModPlayerStatus_t *RenderMOD(short *buf, int len) { int i, s, ch;
		...
		for(ch = 0; ch < mp.channels; ch++) {
			...
			if(pch->sample) {
				...
				if(!pch->muted) {
					...
					sample1 = pch->sample[pch->currentptr];
					sample2 = pch->sample[nextptr];

					assert(pch->currentsubptr < 0x10000, "channel: %d, test %u < 0x10000", ch, pch->currentsubptr);

					sample = (sample1 * (0x10000 - pch->currentsubptr) +
						sample2 * pch->currentsubptr) * pch->volume / 65536;
#else
					sample = pch->sample[pch->currentptr] * pch->volume;
#endif
...
ModPlayerStatus_t *InitMOD(const uint8_t *mod, uint32_t samplerate) { int i, s, ch;
...
ModPlayerStatus_t *JumpMOD(int order) { int i, s, ch;
	int neworder = mp.order, oldorder = 0;
